...
echo <<<_END
<section class="login">
<div class="loginhead">CHECK STUDENT ATTENDENCE</div>
<form action="Wattendance.php" method="post">
ENTER ID: <input type="text" name='HID' placeholder="YYRRRR"><br> <br>
<input class ="enviar" type="submit" name="check" value="CHECK">
</form>
</section>
_END;
...
$HID=$_POST['HID'];
$_POST=array();
$sql = "select ODate,IDate from `hams`.`entryregister`   where HID=$HID group by ODate;";
...
for($itr=$oDate; $itr<$iDate; $itr++)
{
	echo <<<_END
<tr><td>$itr</td><td>A</td></tr>
_END;
}

echo <<<_END
<tr><td>$iDate</td><td>P</td></tr>
_END;
}
