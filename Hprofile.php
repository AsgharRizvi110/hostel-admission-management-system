    ...
    <div class="container">
      ...
      <nav>
        <ul>
          ...
          <li class="current"><a href="Hprofile.php" class="m2">Profile</a></li>
  ...
  <div class="container">
    <aside>
      ...
      <ul class="categories">
		...
		<li class="last"><span><a href="attendance.php">View Attendance</a></span></li>
        <br></br><br></br><br></br><h2><li class="last"><a href="index.php">LOGOUT</a></li></h2>
      </ul>
    </aside>
    <section id="content">
     	<?php
...
if(!isset($_POST['edit'])){
$HID=$_SESSION['user'];
$sql = "SELECT HID,HName,Hemail,Heducation,HNIC, Haddress,RoomNo,FloorNo,messID,enrollDate,Degree FROM `hostelier` where HID=$HID";
$result = $conn->query($sql);

if ($result->num_rows > 0){
	 while($row = $result->fetch_assoc())
	{
		echo "ID:	" . $row["HID"]."<br><br>" . "Name:	" . $row["HName"]."<br><br>" . "Email:	" . $row["Hemail"]. "<br><br>" ."Education:	". $row["Heducation"].
		"<br><br>"."NIC:	". $row["HNIC"]. "<br><br>". "Address:	". $row["Haddress"]. "<br><br>" . "Room No:	". $row["RoomNo"]. "Floor No:	".
		$row["FloorNo"]. "<br><br>"."Mess ID:	". $row["messID"]. "<br><br>". "Enroll Date: ". $row["enrollDate"]. "<br><br>". "Degree: ". $row["Degree"];
	}
...
if(!isset($_POST['edit'])){
echo <<<_END
<form action="Hprofile.php" method="post">
...
$conf=$_POST['confpass'];
$ID=$_SESSION['user'];
$sql = "SELECT Hpassword FROM `hostelier` where HID=$HID";
$result = $conn->query($sql);
$row=$result->fetch_assoc();
$pas=$row['Hpassword'];
if($old===$pas && $new===$conf){
$sql="update hostelier 
set Hpassword= '$new'
where HID=$HID
";
...
<footer>
  <div class="footerlink">
    ...
    <p class="rf">Designed by: <a href="http://www.ictrdf.com/">18K-DB</a></p>
