...
if (!$conn) {
 ...
 $time=$_POST['time'];
$dish=$_POST['dish'];	 
 $sql = "UPDATE `hams`.`mess` SET `Dish` = '$dish' WHERE `mess`.`messID` = '$mID' AND `mess`.`Day` = '$day' AND `mess`.`Time` = '$time';";
 ...
 $time=$_POST['time'];
$dish=$_POST['dish'];	 
 $sql = "delete from `hams`.`mess` WHERE `mess`.`messID` = '$mID' AND `mess`.`Day` = '$day' AND `mess`.`Time` = '$time';";
 if ($conn->query($sql) === TRUE) {
...
_END;
 ...
 $mID=$_POST['mID'];
  ...
  $sql = "Insert into `hams`.`mess` Values ('$mID','$day','$time','$dish');";
 if ($conn->query($sql) === TRUE) {
    echo "UPDATED SUCCESSULLY";
	$_POST = array();
